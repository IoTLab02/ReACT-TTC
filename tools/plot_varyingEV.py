...
in_path = Path("../results/results_varying_EV.csv")
...
#     plt.tight_layout()
#     out_file_name = _y + '_vs_ev.pdf'
#     plt.savefig(out_dir/out_file_name, dpi=300, format='pdf')
#     plt.show()
    
    
df_filtered['Total EV'] = df_filtered['Total EV'].astype(int).astype(str)
df_filtered['Total EV'] = pd.Categorical(df_filtered['Total EV'], ordered=True)
...
for _y in to_plot:
    ...
    plt.xlabel('Total EV', fontsize=20)
    plt.ylabel(_y, fontsize=20)
    # plt.xticks(ticks=sorted(df_filtered1['Total EV'].unique()), labels=sorted(df_filtered1['Total EV'].unique()), fontsize=18)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    ...
    # Save plot
    out_file_name = _y + '_bar_vs_ev.pdf'
    plt.savefig(out_dir / out_file_name, dpi=300, format='pdf')
    plt.show()