...
if __name__ == "__main__":
    ...
    csv_file = os.path.join("..", "results", "results_varying_alpha.csv".format(base_file))
    
    n_ev = 50
    for alpha in (0.1 + 0.2*i for i in range(5)):
        for itr in range(max_itr):
            ...
            with open(csv_file, mode='a', newline='') as file:
                ...
                if not file_exists:
                    writer.writerow(['itr', 'Initial Endowment', 'Method', 'Total EV', 'alpha', 'execution time', 'Sum of assigned indices', 'Total satisfaction PT' ])
                
                writer.writerow([itr, initial_endowment_method, 'ReACT-TTC', n_ev, alpha, elapsed_time_TTC, score_TTC_SI, score_TTC_TS_PT])
                writer.writerow([itr, initial_endowment_method, 'PCD', n_ev, alpha, elapsed_time_PCD, score_PCD_SI, score_PCD_TS_PT])
                writer.writerow([itr, initial_endowment_method, 'PCG', n_ev, alpha, elapsed_time_PCG, score_PCG_SI, score_PCG_TS_PT])
                writer.writerow([itr, initial_endowment_method, 'PCL', n_ev, alpha, elapsed_time_PCL, score_PCL_SI, score_PCL_TS_PT])
                writer.writerow([itr, initial_endowment_method, 'V2VDis', n_ev, alpha, elapsed_time_V2VDis, score_V2VDis_SI, score_V2VDis_TS_PT])
                writer.writerow([itr, initial_endowment_method, 'DAA', n_ev, alpha, elapsed_time_DAA, score_DAA_SI, score_DAA_TS_PT])